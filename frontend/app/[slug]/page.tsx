...
async function getSite(slug: string): Promise<Site | null> {
  try {
    const res = await fetch(`${API_URL}/sites/${slug}`, { cache: 'no-store' });
      ...
      {site.sections
        ...
        .map((section) => {
          ...
          const content = section.type === 'hero'
            ...
            : section.content;
          const inner = <Component key={section.id} content={content} slug={params.slug} />;
...
export async function generateMetadata({ params }: { params: { slug: string } }) {
  const site = await getSite(params.slug);
  return { title: site?.companyName ?? params.slug };
}
