...
const AI_TOOLS_FALLBACK = [
  'Runway Gen-4','Kling 2','Veo 3','Higgsfield AI','Hailuo Minimax 2',
  'Midjourney','Leonardo AI','Krea AI','Suno AI','ElevenLabs',
];
...
interface FooterProps {
  ...
  navItems?: NavLink[];      // Quick Links — mesmos do menu de navegação
  footerLinks?: NavLink[];   // Tools We Use — ferramentas de IA com links externos
}

export function Footer({ footerLogoUrl, navItems, footerLinks }: FooterProps) {
  const hasQuickLinks = navItems && navItems.length > 0;

  const scrollTo = (href: string) => {
    if (!href) { window.scrollTo({ top: 0, behavior: 'smooth' }); return; }
    if (href.startsWith('http') || href.startsWith('/')) { window.open(href, '_blank'); return; }
      ...
      <footer className="relative py-20 bg-gray-900 text-white w-full">
        <div className="container mx-auto px-6 sm:px-8 lg:px-12 max-w-7xl">
          <div className="grid grid-cols-12 gap-12">
            ...
            <div className={`col-span-12 ${hasQuickLinks ? 'md:col-span-3' : 'md:col-span-4'}`}>
              {footerLogoUrl
            ...
            {/* Quick Links — navItems (mesmos do menu de navegação) */}
            {hasQuickLinks && (
              <div className="col-span-12 md:col-span-3">
                <h4 className="font-black text-lg text-white mb-4 uppercase tracking-wide">Quick Links</h4>
                <ul className="space-y-3">
                  {navItems!.map((link, i) => (
                    <li key={i}>
                      <button
                        onClick={() => scrollTo(link.href)}
                        className="text-white/70 hover:text-white transition-colors text-sm font-medium text-left"
                      >
                        {link.label}
                      </button>
                    </li>
            ...
            {/* Tools We Use — footerLinks (ferramentas de IA com links) */}
            <div className={`col-span-12 ${hasQuickLinks ? 'md:col-span-6' : 'md:col-span-8'}`}>
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 gap-6">
                {footerLinks && footerLinks.length > 0
                  ...
                  : AI_TOOLS_FALLBACK.map((tool) => (
                      <div key={tool} className="text-white/80 text-sm font-medium">{tool}</div>
                    ))
                }
              </div>
            </div>
